...
let is_cleanse =
    adapter.kind() == crate::authoring_driver::AuthoringKind::Cleanse;
...
let authoring_ctx = crate::authoring_driver::AuthoringCtx {
    phase,
};
...
let (plan_context, plan_state): (String, PlanState) =
    if is_cleanse {
        let plan = match crate::plan::load_cleanse_plan_any(
            &actx,
        )
        .await
        {
        ...
        {
            let next =
                crate::plan::cleanse_next_authoring_action(&plan)
                    .author_sql_ids();
            ...
            )
        {
            ...
            if let crate::plan::AuthoringNextAction::AuthorSchema(ids) =
                &next_action
            {
                let checklist_item_id = resolve_checklist_item_id(&actx);
                let expected_paths =
                    crate::phase_author_lifecycle::collect_expected_paths(
                        &plan.tasks,
                        ids,
                        |task| task.dataset_id.as_str(),
                        |task| task.expected_model_path.as_deref(),
                ...
                &next_action
            {
                ...
                let completion_snapshot =
                    crate::plan::snapshot_cleanse_completion(&plan);
        ...
        let mut plan = match crate::plan::load_model_plan_any(
            &actx,
        )
        .await
        {
            ...
            )
        {
                ...
                &next_action
            {
                ...
                let completion_snapshot =
                    crate::plan::snapshot_model_completion(&plan);
                if let Some(trigger) = decide_author_validate_trigger(
                    &next_action,
                    &completion_snapshot,
                    phase_guard.last_validate_failed,
...
let single_target_repair_path = if hard_mutation_repair_mode {
    crate::phase_gate::derive_single_target_repair_path(&execution_state)
} else {
    None
};
...
// - Also include any recent validate-fail facts snapshot if present.
{
    ...
    if is_cleanse {
            ...
            crate::plan::load_cleanse_plan(&actx).await
        {
            ...
            {
                batch_relations =
                    crate::facts::dataset_ids_to_fqns(ds);
            }
