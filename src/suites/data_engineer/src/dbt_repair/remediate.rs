...
fn parse_json_from_llm<T: DeserializeOwned>(text: &str) -> Result<T, String> {
    react_core::json_repair::resilient_parse(text)
}
...
async fn best_effort_samples_for_source(
    ...
    let catalog = crate::resolved_config_from_ctx(ctx)
...
fn errors_suggest_uncertainty(errors: &[String]) -> bool {
    let s = crate::dbt_error::compact_brief(errors, 8, 2000).to_lowercase();
...
async fn best_effort_schema_columns_for_relation(
    ...
    let Some(q) = crate::ctx_ext::actx_query(ctx) else {
        return vec![];
    };
...
pub async fn remediate_dbt_failures_grounded_with_llm(
    ...
    for rel in crate::project_fs::CORE_PROJECT_CONTEXT_FILES.iter() {
    ...
    for f in files.iter() {
        ...
        for r in crate::naming::extract_ref_calls(content).into_iter() {
            ref_names.insert(r);
        }
    }
    ...
    let error_brief = crate::dbt_error::compact_brief(errors, 8, 2400);
...
pub async fn remediate_unresolved_columns_with_llm(
    ...
    for k in keys.iter() {
        ...
        let sources = crate::naming::extract_source_calls(&text);
    ...
    let error_brief = crate::dbt_error::compact_brief(errors, 6, 900);
