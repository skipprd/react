    ...
    async fn call(&self, args: Value, ctx: &AgentCtx) -> Result<Value, String> {
        ...
        if !is_model_agent {
            for h in all_hits.iter_mut() {
                ...
                if h.item.kind == react_core::providers::ChunkKind::Other("artifact".to_string()) {
        ...
        all_hits.retain(|h| h.item.kind != react_core::providers::ChunkKind::Other("dbt_example".to_string()));
