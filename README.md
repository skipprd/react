...
- **File**: `src/runtime/src/ws/server.rs`
- Responsibilities:
...
- **Files**: `src/suites/react-suites/src/*`
- A **suite** owns:
...
- **File**: `src/core/src/agent/mod.rs`
- Responsibilities:
...
- **Threads**: `src/core/src/session/mod.rs` (`ThreadStore` persists steps as JSON via `StorageAdapter` + `Keyspace`)
- **Artifacts/catalog**: stored via `StorageAdapter` at keys derived from `Keyspace`

### Providers (capabilities injection)

Providers live under `src/runtime/src/providers/*` and are injected via `SuiteCtx`:
...
The concrete providers used by the CLI server (`src/runtime/src/main.rs`) determine whether the runtime uses Athena, etc. Suites and the agent loop remain provider-agnostic.
